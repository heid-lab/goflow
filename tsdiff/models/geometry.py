...
def get_distance(pos, edge_index):
    """
    Calculate the Euclidean distance between pairs of nodes in a graph.

    Returns:
        torch.Tensor: 
            A 1D tensor of shape (E,) where each element represents the Euclidean 
            distance between the source and target nodes of an edge.
    """
...
def convert_cluster_score_d(
    ...
    Args:
        ...
        subgraph_index:     (N, )
    """
    cluster_score_pos = eq_transform(
        cluster_score_d, cluster_pos, cluster_edge_index, cluster_edge_length
    )  # (C, 3)
...
def get_angle(pos, angle_index):
    ...
    length_prod = torch.norm(v1, dim=-1, keepdim=True) * torch.norm(
        v2, dim=-1, keepdim=True
    )  # (A, 1)
...
def get_dihedral(pos, dihedral_index):
    ...
    length_prod = torch.norm(n1, dim=-1, keepdim=True) * torch.norm(
        n2, dim=-1, keepdim=True
    )
